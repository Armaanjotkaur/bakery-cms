...
export default function CategoriesPage() {
  ...
  return (
    <main className="p-8 max-w-2xl mx-auto">
      <h1 className="text-2xl font-semibold mb-6 text-stone-900">Categories</h1>
      ...
      <form onSubmit={handleCreate} className="flex gap-3 mb-6">
        <input
          ...
          className="flex-1 border border-rose-200 rounded-md px-3 py-2 text-stone-900 focus:outline-none focus:ring-2 focus:ring-rose-400"
        />
        <select
          value={type}
      ...
      <table className="w-full text-sm border border-rose-100 rounded-lg overflow-hidden">
        ...
        <tbody>
          {categories.map((c) => (
            <tr key={c._id} className="border-t border-rose-50">
              <td className="p-3 text-stone-900">{c.name}</td>
              <td className="p-3 text-stone-500">{c.type}</td>
              <td className="p-3">
                <button
      ...
      {categories.length === 0 && <p className="text-stone-500 text-center py-8">No categories yet.</p>}
    </main>
  );
}
